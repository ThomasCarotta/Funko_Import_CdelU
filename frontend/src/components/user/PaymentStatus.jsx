...
const PaymentStatus = () => {
  ...
  useEffect(() => {
    if (isPurchaseRegistered) return;
    ...
    if (status === "approved") {
      ...
      fetch("http://127.0.0.1:8000/api/auth/payment-success/", {
        ...
        body: JSON.stringify({
          ...
          descuento: discountAmount, // Incluir el descuento
          items: cart.map((item) => ({
      ...
      .finally(() => {
        // Limpiar sessionStorage y redirigir al home
        sessionStorage.removeItem("cart");
        sessionStorage.removeItem("discountAmount");
        navigate("/");
      });
    } else {
      // Si el pago no fue aprobado, mostrar mensaje de error
      Swal.fire({
  ...
  }, [location, navigate, isPurchaseRegistered]);
